...
function cmeboss_display_wishlist_items_details_and_cancel_order_button() {
    ...
    if ($wishlist_items_count > 0) {
        echo '<p>您的願望清單中還有 ' . $wishlist_items_count . ' 件商品待處理：</p>';
        echo '<ul>';
    ...
    if (!empty($wishlist_items)) {
        foreach ($wishlist_items as $wishlist_item) {
            ...
            foreach (WC()->cart->get_cart() as $cart_item) {
                if ($cart_item['product_id'] === $wishlist_item['product_id'] &&
                    $cart_item['variation_id'] === $wishlist_item['variation_id']) {
                    $is_in_cart = true;
                    break;
                }
            }
        ...
        WC()->session->set('wishlist_items', array()); // 清空願望清單
